...
fun InProgressScreen(
    ...
    val visibleBooks by remember(query, selectedFilter) {
        mutableStateOf(allInProgress.filter { matchesFilter(it) })
    }
    ...
    Scaffold { padding ->
        Column(
            ...
            OutlinedTextField(
                ...
                label = { Text("Search in-progress books") },
            ...
            if (visibleBooks.isEmpty()) {
                Box(modifier = Modifier.fillMaxSize(), contentAlignment = androidx.compose.ui.Alignment.Center) {
                    Text("No books in progress yet", style = MaterialTheme.typography.bodyLarge)
                }
            } else {
                LazyVerticalGrid(
